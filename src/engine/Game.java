...
public class Game {
	...
	public void attack(Direction d)
		...
		if (found) {
			...
			controller.Controller.playInstance("Sounds/Punch.wav");
		}
	}
	...
	public void useLeaderAbility() throws LeaderAbilityAlreadyUsedException, LeaderNotCurrentException {
		...
		if (firstPlayer.getLeader() == c) {
			...
			if(c.getName().equals("Slim Abdennadher"))
			{
				for(int i= secondPlayer.getTeam().size()-1 ; i>=0 ; i--)
				{
			...
			if(c.getName().equals("Slim Abdennadher"))
			{
				for(int i= firstPlayer.getTeam().size()-1 ; i>=0 ; i--)
				{
	...
	public void castAbility(Ability a)
		...
		a.execute(targets);
		controller.Controller.playInstance("Sounds/"+ a.getName()+".wav");
